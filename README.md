...
FeiShu-WoL 是一个运行在 OpenWrt/ImmortalWrt 或普通 Linux 服务器上的 Wake-on-LAN 服务。它可以通过飞书、Telegram 或自建 Web 中转服务接收远程开机指令，并在局域网内发送 WoL Magic Packet。
...
- OpenWrt LuCI：后台 `服务` → `FeiShu-WoL` 可完成全部常用配置。
